...
<head>
    ...
    <title>Tambah Kategori - Admin MyPengembalianBarangKu</title>
...
<body class="bg-stone-50 font-sans text-stone-800">
    ...
    <main class="container mx-auto px-4 py-8">
        <div class="max-w-2xl mx-auto bg-white rounded-2xl shadow-sm border border-stone-100 p-8">
            <h2 class="text-2xl font-sans font-bold text-stone-800 mb-6">Tambah Kategori Baru</h2>

            <form action="{{ route('admin.categories.store') }}" method="POST">
                @csrf

                <div class="mb-4">
                    <label class="block text-stone-700 font-medium mb-1">Nama Kategori</label>
                ...
                <div class="mb-6">
                    <label class="block text-stone-700 font-medium mb-1">Deskripsi (Opsional)</label>
                    <textarea name="description" rows="3"
                        class="w-full rounded-lg border-stone-300 focus:border-primary-500 focus:ring focus:ring-primary-200 transition">{{ old('description') }}</textarea>
                    @error('description') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                </div>

                <div class="flex gap-3">
                    <button type="submit"
                        class="bg-primary-700 hover:bg-primary-800 text-white px-6 py-2 rounded-lg font-medium transition">Simpan</button>
